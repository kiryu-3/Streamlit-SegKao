...
def upload_csv():
    ...
    if st.session_state['upload_csvfile'] is not None:
        ...
        try:
            df = pl.read_csv(io.TextIOWrapper(file_data, encoding="utf-8"), has_header=True)
            st.session_state["ja_honyaku"] = False
        except UnicodeDecodeError:
            # UTF-8で読み取れない場合はShift-JISエンコーディングで再試行
            df = pl.read_csv(io.TextIOWrapper(file_data, encoding="shift-jis"), has_header=True)
            # df = pl.read_csv(io.BytesIO(file_data), encoding="shift-jis", engine="python")
